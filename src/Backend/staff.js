...
const getTutorwardStudents = async (req, res) => {
  try {
      let { tutorward } = req.query;  // Extracting tutorward from query params
      console.log(tutorward+'hi')
      if (!tutorward) {
          return res.json([]);
      }

      // Ensure tutorward is an array
      if (!Array.isArray(tutorward)) {
          tutorward = tutorward.split(','); // Convert CSV string to array
      }
...
const getStudentCourses = async (req, res) => {
  try {
      ...
      const completedCoursesQuery = `
          SELECT cc.course_code, cd.name, cc.score, cc.grade
      ...
      const ongoingCoursesQuery = `
          SELECT cr.course_code, cd.name, cd.credits
...
try {
  ...
  console.log(tutorEmail)
    ...
    // Fetch tutor's batch year
    const tutorQuery = await pool.query(
        "SELECT batch FROM staff_info WHERE regno = $1",
        [tutorEmail]
    );
    ...
    const batchYear = tutorQuery.rows[0].batch;
...
try {
  const regno = req.params.tutorEmail; // ✅ Correct way to get the parameter
    const { regnos } = req.body; // Expecting an array of regnos
    if (!regnos || regnos.length === 0) {
        return res.status(400).json({ message: "No students selected" });
    }

    const tutorQuery = await pool.query(
      "SELECT name FROM staff_info WHERE regno = $1 ",
      [regno]
  );
  const tutorName = tutorQuery.rows[0].name;
...
try {
  const tutorEmail = req.params.tutorEmail; // Tutor's regno (email)
  const { studentRegNo } = req.body; // Student regno to remove
  if (!studentRegNo) {
...
try {
  ...
  `;
  const tutorResult = await pool.query(tutorQuery, [regno]);
  ...
  const verificationQuery = `
    SELECT * FROM verification_details WHERE student_regno = ANY($1)
  `;
...
try {
    const result = await pool.query("SELECT code, name, no_of_weeks, credits FROM course_details");
    res.json(result.rows);
} catch (err) {
    console.error(err);
    res.status(500).send("Server error");
...
try {
    ...
    const result = await pool.query(
        "SELECT cr.student_regno, cr.score ,cr.grade ,s.name FROM course_registration cr join students_info s on cr.student_regno=s.regno WHERE course_code = $1",
        [code]
    );
    res.json(result.rows);
} catch (err) {
...
try {
    ...
    const scores = students.rows.map(s => s.score !== null ? parseFloat(s.score) : null);
    ...
    if (totalStudents < 25) {
        ...
        scores.forEach(score => {
            ...
            else if (score >= 50) grades[score] = 'B';
            else grades[score] = 'RA';
        });
...
module.exports={getStaff,addStaff,editStaff,deleteStaff,editProfileStaff,getTutorwardStudents,getStudentCourses ,getEligibleStudents,addStudentsToTutorward,actionVerification,getAllCourses,getCoursesSearch,getCoursesFilter,getCourseStudents,addGrade,removeStudentFromTutorward};
