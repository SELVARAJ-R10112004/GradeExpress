...
const InchargeHomePage = ({ user, setUser,logout }) => {
  ...
  const [deleteRegNo, setDeleteRegNo] = useState(""); // Input for deleting student
  const navigate=useNavigate();
  useEffect(() => {
    if (user.designation!="Tuytor") {
      axios
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      ...
      setStudentsList(studentsList.filter(student => student.regno !== deleteRegNo));
      ...
      <div>
        <h1 className="student-head">Tutor Ward</h1>
          <button className="btn btn-primary d-flex justify-content-end"
            ...
            style={{ marginLeft: "100px" }} // Adjusted margin
            >
            Add Students
        </button>
